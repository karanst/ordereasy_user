...
import 'package:eshop_multivendor/Model/get_posts_model.dart';
...
class PostsScreen extends StatefulWidget {
  ...
  const PostsScreen(
      {Key? key, this.sellerId})
      : super(key: key);

  @override
  State<StatefulWidget> createState() => StatePosts();
}

class StatePosts extends State<PostsScreen> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: getAppBar("Posts", context),
        key: _scaffoldKey,
  ...
  List<Widget> _buildDots(int index) {
    List<Widget> dots = [];
    for (int i = 0; i < tempList[index].images!.length; i++) {
      dots.add(
        Container(
    ...
    apiBaseHelper.postAPICall(getPostsApi, parameter).then((getdata) {
      ...
      if (!error) {
        ...
        setState(() {
          ...
          var data = getdata["data"];
          tempList =
              (data as List).map((data) => new Posts.fromJson(data)).toList();
        //
        ...
        children: [
          ...
          Expanded(
            child: tempList.length == 0
              ...
              itemBuilder: (context, index) {
                return (index == tempList.length && isLoadingmore)
