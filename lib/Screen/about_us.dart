...
import 'package:eshop_multivendor/Model/about_slider_model.dart';
...
class AboutUsScreen extends StatefulWidget {
  ...
  const AboutUsScreen({Key? key, this.title}) : super(key: key);
...
class StateAboutUs extends State<AboutUsScreen> with TickerProviderStateMixin {
  ...
  getAboutSlider() async{
    var headers = {
      'Cookie': 'ci_session=85b3906c1dd7f97b39f2fa32eecb20750a5f6c0e'
    };
    var request = http.MultipartRequest('POST', Uri.parse('${baseUrl}get_slider_about'));
    ...
    if (response.statusCode == 200) {
      ...
      final jsonResponse = AboutSliderModel.fromJson(json.decode(finalResult));
      setState(() {
        sliderList = jsonResponse.data!;
      });
    }
    else {
                  ...
                  return Builder(
                    builder: (BuildContext context) {
                      return Container(
                          ...
                          // child:
                          // CachedNetworkImage(
                          //   imageUrl:  "https://developmentalphawizz.com/mt/${i.image}",
                          // )
                        child:   ClipRRect(
                              borderRadius: BorderRadius.circular(10),
                              child:
                              Image.network(
                                "https://developmentalphawizz.com/mt/${i.image}",
                                fit: BoxFit.fill,
                              )
                      ),
                      );
